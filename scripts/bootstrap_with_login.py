...
def find_latest_peer_config():
    ...
    for p in glob.glob(os.path.join(CONFIG_DIR, "peer_*.yaml")):
        ...
        if base.startswith("peer_") and base.endswith(".yaml"):
            try:
                n = int(base[5:7])  # supports peer_01.yaml..peer_99.yaml
            except ValueError:
                # fallback try longer numbers
                try:
                    n = int(base[5:-5])
                except Exception:
                    continue
...
def main():
    ...
    try:
        ...
        # Find latest peer config (should be the one just created)
        new_cfg_path = find_latest_peer_config()
        if not new_cfg_path or new_cfg_path in (PEER1, PEER2):
            print("No se encontró el nuevo peer config.")
            sys.exit(1)

        new_cfg = load_yaml(new_cfg_path)
        new_port = int(new_cfg.get("rest_port"))
        new_ip = new_cfg.get("ip", "127.0.0.1")
        new_addr = f"{new_ip}:{new_port}"

        # Start new node
        print(f"Iniciando nuevo nodo con {new_cfg_path} en {new_addr}...")
        p_new = start_node(new_cfg_path)
